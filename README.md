...
- **Aviso de possível reimportação:** antes de confirmar um CSV demonstrativo, a prévia compara o nome do arquivo e o valor válido com os envios anteriores da mesma conciliação. Quando encontra nome igual, mostra os valores lado a lado e exige ciência explícita para habilitar a confirmação. Nome igual é um alerta, não prova de duplicidade. A regra oficial de somar ou substituir envios continua indefinida.
- **Desfazer a última ação:** após um lançamento manual na Operação ou uma associação de recebimento no extrato, aparece uma ação para reverter aquela alteração. O protótipo restaura os totais e o vínculo anterior quando houver. O desfazer existe somente na sessão atual e para a última ação reversível exibida; não é um histórico de desfazer global.
- **Qualidade dos dados:** a Visão geral lista recebimentos sem fonte e lançamentos sem artista ou referência, com acesso à conferência do extrato ou da fonte. A lista é derivada dos dados fictícios da competência selecionada e não avalia todas as regras de qualidade necessárias em produção.
...
- A prévia do CSV é sintética e não lê o arquivo escolhido; por isso a comparação de duplicidade também usa nome e valores fictícios. No projeto oficial, uma detecção confiável deverá usar metadados e conteúdo validados pela API antes de confirmar a importação.
- As ações de desfazer alteram apenas o adaptador em memória; no projeto oficial, exigem operações reversíveis autorizadas pela API, registro de auditoria e tratamento de concorrência.
