...
export default function ReconciliationPage({ section, onSectionChange, onReviewBankStatement, entity, onEntityChange, period, onPeriodChange, statementId, onStatementChange, selectedSourceId, onSelectedSourceChange, operationFilter, onOperationFilterChange, operationQuery, onOperationQueryChange, savedOperationViews, onSaveOperationView, onDeleteOperationView, onOperationDraftDirtyChange, sessionNotes, onAddSessionNote, sessionActions, onAddSessionAction, sources, sourcesError, historyOpenRequest }: Props) {
  ...
  useEffect(() => {
    if (!statementId) { setBankStatement(undefined); return; }
    const controller = new AbortController();
    setBankStatement(undefined);
    getBankStatement(statementId, controller.signal).then(setBankStatement).catch(() => { if (!controller.signal.aborted) setBankStatement(undefined); });
    return () => controller.abort();
  }, [statementId]);
  useEffect(() => {
    if (!existingId) { setView(undefined); setOpening(false); setIngestionBatches([]); return; }
    const controller = new AbortController();
    setView(undefined); setIngestionBatches([]); setError(""); setOpening(true);
    ...
    getIngestionBatches(existingId, controller.signal).then(setIngestionBatches)
      .catch(() => { if (!controller.signal.aborted) setIngestionBatches([]); });
    return () => controller.abort();
  }, [existingId]);
