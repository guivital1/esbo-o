    ...
    {mode === "history" && <>
      ...
      <section className="table-section"><div className="table-toolbar"><div><h2>Extratos salvos</h2><p>{visibleHistory.length} registro(s) no histórico</p></div></div>
        {historyLoading ? <StatusNotice tone="loading" className="bank-history-notice">Carregando histórico…</StatusNotice> : visibleHistory.length === 0 ? <p className="empty">Nenhum extrato encontrado. Use “+ Importar extrato” para começar.</p> :
          <div className="table-wrap"><table className="history-table"><thead><tr><th>Competência</th><th>Empresa</th><th>Banco</th><th>Movimentos</th><th>Total</th><th>Status</th><th>Ações</th></tr></thead><tbody>{visibleHistory.map((item) =>
            <tr key={item.id_extrato}><td>{monthLabel(item.period)}</td><td>{item.entity}</td><td>{item.bank}</td><td>{item.displayed_count}</td><td className="amount">{money.format(Number(item.displayed_total))}</td><td><span className={`source-status ${item.status === "validated" ? "is-active" : "is-pending"}`}>{item.status_label}</span></td><td><button className="source-action" type="button" onClick={() => openSaved(item.id_extrato)}>Abrir</button></td></tr>)}</tbody></table></div>}
      </section></>}
